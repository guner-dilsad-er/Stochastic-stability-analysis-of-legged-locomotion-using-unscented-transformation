...
% height_sweep(end)=[];
function Method_Visualization(noise1_var,height_sweep)
...
figure()
ylim([0.7 1.05])
[V,D]=eig(T_montecarlo);
p1=plot(real(diag(D(1:4,1:4))),'b.');
real(diag(D(1:4,1:4)))
hold on
[V,D]=eig(T_sys);
p2=plot(real(diag(D(1:4,1:4))),'bx');
real(diag(D(1:4,1:4)))
xlim([0 5])
plot(xlim, [real(diag(D(2,2))) real(diag(D(2,2)))], 'b--')
plot(xlim, [real(diag(D(3,3))) real(diag(D(3,3)))], 'b--')
plot(xlim, [real(diag(D(4,4))) real(diag(D(4,4)))], 'b--')
[V,D]=eig(T_unsc);
p3=plot(real(diag(D(1:4,1:4))),'rx');
real(diag(D(1:4,1:4)))
plot(xlim, [real(diag(D(2,2))) real(diag(D(2,2)))], 'r--')
plot(xlim, [real(diag(D(3,3))) real(diag(D(3,3)))], 'r--')
plot(xlim, [real(diag(D(4,4))) real(diag(D(4,4)))], 'r--')
[V,D]=eig(T_ext);
p4=plot(real(diag(D(1:4,1:4))),'r.');
real(diag(D(1:4,1:4)))
grid on
ylim([0.7 1.02])
xlim([0.8 4.2])
legend([p1,p2,p3,p4],'Monte Carlo','Systematic Experiments','Unscented Transformation','Linearization','Location','southwest')
title('Eigenvalue Comparison','Interpreter','latex')
xticks([1,2,3,4])
xticklabels({'$$\lambda_1$$','$$\lambda_2$$','$$\lambda_3$$','$$\lambda_4$$'})
grid on
xlabel('Eigenvalues')
ylabel('Value')
...
grid on
legend('Unscented Transformation','Systematic Experiments','Monte Carlo','Linearization')
...
suptitle('Comparison of Output Distributions')
...
plot(height_sweep(2:end),ext_mean_list-mc_means,'.k'),hold on
plot(height_sweep(2:end),mc_std-sqrt(ext_var_list),'.r'),hold on
plot(height_sweep(2:end),mean_list-mc_means,'.b'),hold on
plot(height_sweep(2:end),mc_std-sqrt(var_list),'.g'),hold on  
...
phi(1)=0;
s=surfc(H1,H2,T_unsc.*phi/sum(phi));
s(1).EdgeColor = 'none';
%view(135,45)
view(0,90)
s(2).EdgeColor = 'r';
